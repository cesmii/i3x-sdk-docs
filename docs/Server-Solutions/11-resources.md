# Resources and Community

## Official Resources

### CESMII

- **Main Website**: https://www.cesmii.org
- **RFC Specification**: https://github.com/cesmii/API
- **Reference Implementation**: https://github.com/cesmii/API/tree/prototypes
- **Issue Tracker**: https://github.com/cesmii/API/issues
- **Demo Server**: https://i3x.cesmii.net
- **API Documentation**: https://i3x.cesmii.net/docs

### Smart Manufacturing Profiles

- **SM Profiles Repository**: https://github.com/cesmii/SMProfiles
- **Profile Designer**: https://profiles.cesmii.org
- **Cloud Library**: https://uacloudlibrary.opcfoundation.org
- **Marketplace**: https://marketplace.cesmii.org

### GraphQL API (Legacy)

- **GraphQL API Docs**: https://github.com/cesmii/GraphQL-API
- **Connector Documentation**: https://github.com/cesmii/Connector

## Related Standards

### OPC UA

- **OPC Foundation**: https://opcfoundation.org
- **OPC UA Specification**: https://opcfoundation.org/developer-tools/specifications-unified-architecture
- **OPC UA Cloud Library**: https://uacloudlibrary.opcfoundation.org
- **OPC UA GitHub**: https://github.com/OPCFoundation

### Asset Administration Shell (AAS)

- **IDTA**: https://industrialdigitaltwin.org
- **AAS Specification**: https://industrialdigitaltwin.org/content-hub/aasspecifications
- **Platform Industrie 4.0**: https://www.plattform-i40.de

### Other Standards

- **MQTT**: https://mqtt.org
- **Sparkplug**: https://www.eclipse.org/tahu/spec/Sparkplug%20Topic%20Namespace%20and%20State%20ManagementV2.2-with%20appendix%20B%20format%20-%20Eclipse.pdf
- **MTConnect**: https://www.mtconnect.org
- **ISA-95**: https://www.isa.org/standards-and-publications/isa-standards/isa-standards-committees/isa95

## Community Support

### Contact Information

- **RFC Email**: rfc@cesmii.org
- **Developer Support**: devops@cesmii.org
- **General Inquiries**: info@cesmii.org

### Discussion Forums

- **GitHub Discussions**: https://github.com/cesmii/API/discussions
- **LinkedIn Group**: CESMII - The Smart Manufacturing Institute

### Training and Education

- **CESMII YouTube Channel**: https://www.youtube.com/@CESMII_SM
- **Training Resources**: https://www.cesmii.org/education-workforce
- **Webinars**: https://www.cesmii.org/events

## Development Tools

### API Development

- **Postman Collection**: [Coming Soon]
- **Insomnia Workspace**: [Coming Soon]
- **cURL Examples**: Available in docs/examples/

### Code Generators

- **OpenAPI Generator**: https://openapi-generator.tech
- **Swagger Codegen**: https://swagger.io/tools/swagger-codegen

### Testing Tools

- **pytest**: https://pytest.org
- **pytest-flask**: https://pytest-flask.readthedocs.io
- **Locust**: https://locust.io (load testing)
- **Postman**: https://www.postman.com

### Documentation Tools

- **Swagger UI**: https://swagger.io/tools/swagger-ui
- **ReDoc**: https://redocly.com/redoc
- **Slate**: https://github.com/slatedocs/slate

## Libraries and SDKs

### Python

- **Flask**: https://flask.palletsprojects.com
- **FastAPI**: https://fastapi.tiangolo.com
- **SQLAlchemy**: https://www.sqlalchemy.org
- **Marshmallow**: https://marshmallow.readthedocs.io

### .NET

- **ASP.NET Core**: https://docs.microsoft.com/en-us/aspnet/core
- **Entity Framework**: https://docs.microsoft.com/en-us/ef
- **OPC UA .NET**: https://github.com/OPCFoundation/UA-.NETStandard

### JavaScript/Node.js

- **Express**: https://expressjs.com
- **Fastify**: https://www.fastify.io
- **TypeORM**: https://typeorm.io
- **node-opcua**: https://node-opcua.github.io

### Java

- **Spring Boot**: https://spring.io/projects/spring-boot
- **Hibernate**: https://hibernate.org
- **Eclipse Milo**: https://github.com/eclipse/milo (OPC UA)

## Example Implementations

### Reference Implementations

- **Python/Flask**: https://github.com/cesmii/API/tree/prototypes/python
- **.NET**: https://github.com/cesmii/API/tree/prototypes/dotnet
- **Node.js**: https://github.com/cesmii/API/tree/prototypes/nodejs

### Community Implementations

[List will be populated as community contributes implementations]

## Learning Resources

### Tutorials

- [Getting Started with i3X](docs/tutorials/getting-started.md)
- [Building Your First Server Implementation](docs/tutorials/first-implementation.md)
- [Working with Smart Manufacturing Profiles](docs/tutorials/sm-profiles.md)
- [Implementing Authentication](docs/tutorials/authentication.md)

### Blog Posts

- [Introduction to the i3X API](https://www.cesmii.org/blog/cm-api-intro)
- [Why We Need a Common Manufacturing API](https://www.cesmii.org/blog/why-common-api)

### Videos

- [CESMII YouTube Channel](https://www.youtube.com/@CESMII_SM)
- [API Overview Webinar](https://www.cesmii.org/webinars)

## Conference and Events

### Annual Events

- **CESMII Annual Conference**
- **Smart Manufacturing Experience**
- **OPC Foundation DevCon**

### Webinars

Check https://www.cesmii.org/events for upcoming webinars on:
- API implementation
- Smart Manufacturing Profiles
- Best practices
- Use cases and demos

## Contributing

### How to Contribute

1. **Code Contributions**
   - Submit pull requests to https://github.com/cesmii/API
   - Follow the contribution guidelines
   - Add tests and documentation

2. **Documentation**
   - Improve existing documentation
   - Add examples
   - Translate documentation

3. **Bug Reports**
   - Use GitHub Issues
   - Provide detailed reproduction steps
   - Include version information

4. **Feature Requests**
   - Discuss in GitHub Discussions first
   - Submit detailed proposals
   - Consider backward compatibility

### Governance

- **Working Group**: CESMII/VDMA Joint Working Group
- **Process**: W3C-style standards development
- **License**: BSD 3-Clause

## Success Stories

### Case Studies

[Case studies will be added as implementations are deployed]

### Testimonials

[User testimonials will be added]

## Roadmap

### Current Focus (2025)

- Finalize v1.0 specification
- Reference implementations
- Community engagement
- Documentation improvements

### Future Plans

- Additional protocol bindings
- Enhanced security features
- Advanced query capabilities
- Event subscriptions
- GraphQL alternative endpoint

## Getting Help

### Before Asking for Help

1. Check the [FAQ](docs/faq.md)
2. Search [GitHub Issues](https://github.com/cesmii/API/issues)
3. Review the [documentation](https://github.com/cesmii/API)
4. Check [examples](docs/examples/)

### How to Ask for Help

1. Provide context and details
2. Include code samples
3. Describe what you've tried
4. Share error messages
5. Specify versions used

### Support Channels

- **Email**: rfc@cesmii.org (for RFC feedback)
- **Email**: devops@cesmii.org (for technical support)
- **GitHub Issues**: For bug reports
- **GitHub Discussions**: For questions and discussions

## Acknowledgments

This API specification was developed through collaboration between:
- CESMII - The Smart Manufacturing Institute
- VDMA (German Engineering Federation)
- Industry partners and contributors

Special thanks to all contributors and reviewers.

## License

The i3X API specification is licensed under the BSD 3-Clause License.

See [LICENSE](LICENSE) for details.
